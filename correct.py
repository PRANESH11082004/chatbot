import os
# 🟢 CRITICAL FIX: Allow Google Auth to run on HTTP (localhost)
...
def get_gemini_client():
    ...
    # 1. Check for keys in the specific format GEMINI_API_KEY_1...
    if not GEMINI_API_KEYS:
        # Fallback: Try standard 'GEMINI_API_KEY' or 'GOOGLE_API_KEY'
        fallback_key = os.getenv("GEMINI_API_KEY") or os.getenv("GOOGLE_API_KEY")
        if fallback_key:
            return genai.Client(api_key=fallback_key)
        
        st.error("❌ No Gemini API keys found! Please add GEMINI_API_KEY_1 to your .env file.")
        return None

    active_index = get_active_api_key_index()
    # Ensure index is within bounds (in case keys were removed)
    if active_index >= len(GEMINI_API_KEYS):
        active_index = 0
        set_active_api_key_index(0)
...
def register_google_user(username, email, profile_pic):
    """Register or log in user via Google data and store profile image"""
    client = get_mongo_client()
    ...
    if not user:
        ...
        user_data = {
            "username": username,
            "email": email,
            "profile_pic": profile_pic, # Save the URL captured from Google
...
[data-testid="stTable"] tr:nth-child(1) { background-color: rgba(255, 215, 0, 0.3) !important; border-left: 5px solid #FFD700; } /* Gold */
[data-testid="stTable"] tr:nth-child(2) { background-color: rgba(192, 192, 192, 0.3) !important; border-left: 5px solid #C0C0C0; } /* Silver */
[data-testid="stTable"] tr:nth-child(3) { background-color: rgba(205, 127, 50, 0.3) !important; border-left: 5px solid #CD7F32; } /* Bronze */
...
/* 2. Style the header text as a professional title */
[data-testid="stExpander"] summary p {
    font-size: 20px !important; /* Slightly smaller to ensure fit */
...
/* 3. Clean up the box layout now that the arrow is gone */
[data-testid="stExpander"] summary {
    display: block !important; /* Removes flex alignment for standard text flow */
...
def save_quiz_result(username, quiz_id, score, total_questions, answers, time_taken):
    """Save quiz result with time tracking for leaderboard"""
    client = get_mongo_client()
    if not client: 
        return False
    ...
    result_data = {
        ...
        "time_taken": time_taken, # Added for Leaderboard
...
def get_leaderboard(quiz_id):
    """Fetch top performers for a specific quiz (Sorted by Score then Time)"""
    client = get_mongo_client()
    if not client: 
        return []
    db = client[DB_NAME]
    # Sort: Higher score first, then lower time taken (faster)
    return list(db.quiz_results.find({"quiz_id": str(quiz_id)}).sort([("score", -1), ("time_taken", 1)]).limit(10))
...
def show_auth_page():
    ...
    # --- 2. MANUAL LOGIN LOGIC (The Fix) ---
    # We intercept the 'code' from the URL and verify it manually.
    # This bypasses the library's cookie requirement entirely.
    if "code" in st.query_params:
        try:
            # A. Exchange the authorization code for an access token
            ...
            if response.status_code == 200:
                ...
                # B. Use the token to get user profile info
                ...
                if user_response.status_code == 200:
                    ...
                    # C. Register/Update in your MongoDB
                    success, user_data = register_google_user(name, email, picture)
                    
                    if success:
                        # D. Force the session to be logged in
                        ...
                        # Clear the URL parameters to prevent re-login loops
                        st.query_params.clear()
                        st.rerun()
            else:
                # If Google rejects the code (e.g., it was already used)
                st.error("Login session expired. Please click 'Sign in' again.")
                st.query_params.clear()
                
        except Exception as e:
            st.error(f"Manual Login Error: {e}")

    # --- 3. DISPLAY THE BUTTON ---
    # We still use the Authenticator class just to render the button correctly.
    # We DO NOT rely on its login() method for the actual logic anymore.
    authenticator = Authenticate(
        secret_credentials_path="google_credentials.json",
        cookie_name="legal_advisor_cookie",
        cookie_key=os.getenv("GOOGLE_CLIENT_SECRET"),
        redirect_uri="http://localhost:8501"
    )

    st.markdown("<h1 style='text-align: center;'>Legal Advisor</h1>", unsafe_allow_html=True)
    ...
    # --- MANUAL LOGIN TABS (Existing Code) ---
...
# ---------------------------
# Load and save Legal Datasets from JSON
# ---------------------------

def save_legal_datasets(datasets):
    """Save the updated datasets back to the JSON file"""
    try:
        json_path = os.path.join(os.path.dirname(__file__), 'legal_datasets.json')
        with open(json_path, 'w', encoding='utf-8') as f:
            json.dump(datasets, f, indent=4)
        # 💡 CRITICAL: Clear the cache so the app uses the new data immediately
        load_legal_datasets.clear() 
        return True
    except Exception as e:
...
def query_gemini(user_question, category, retry_count=0):
    ...
    try:
        if not gemini_client:
            return "⚠️ System Error: Gemini Client not connected. Check API keys."
...
def admin_quiz_panel():
    ...
    tab1, tab2, tab3 ,tab4 = st.tabs(["Create Quiz", "Manage Quizzes", "API Key Management", "Manage Laws"])
        ...
        if not quizzes:
            ...
            for quiz in quizzes:
                with st.expander(f"📝 {quiz['title']} ({quiz['category']})"):
                    ...
                    st.write(f"**Difficulty:** {quiz.get('difficulty', 'N/A')} | **Time:** {quiz.get('time_limit', '10')} mins")
    ...
    with tab4:
        st.subheader("Legal Dataset Editor (CRUD)")
        current_data = load_legal_datasets()
        ...
        with st.expander("", expanded=False):
            with st.form("add_law_form"):
                ...
                new_prompt = st.text_input("Question/Prompt")
                new_completion = st.text_area("Legal Answer")
                if st.form_submit_button("Save Entry"):
                    if new_prompt and new_completion:
                        current_data[new_cat].append({"prompt": new_prompt, "completion": new_completion})
                        if save_legal_datasets(current_data):
                            st.success("Entry added!")
                            st.rerun()
        
        # --- READ, UPDATE, DELETE EXISTING ---
        st.divider()
        selected_cat = st.selectbox("View Category", categories)
        for i, item in enumerate(current_data[selected_cat]):
            with st.container():
                col1, col2 = st.columns([4, 1])
                with col1:
                    u_p = st.text_input(f"Q{i+1}", value=item['prompt'], key=f"up_{selected_cat}_{i}")
                    u_c = st.text_area(f"A{i+1}", value=item['completion'], key=f"ua_{selected_cat}_{i}")
                with col2:
                    st.write("") # Spacer
                    if st.button("💾", key=f"save_{selected_cat}_{i}", help="Update"):
                        current_data[selected_cat][i] = {"prompt": u_p, "completion": u_c}
                        save_legal_datasets(current_data)
                        st.rerun()
                    if st.button("🗑️", key=f"del_{selected_cat}_{i}", help="Delete"):
                        current_data[selected_cat].pop(i)
                        save_legal_datasets(current_data)
                        st.rerun()
            st.divider()
...
def user_quiz_panel():
    ...
    with tab1:
        ...
        if "show_quiz_results" in st.session_state and st.session_state.show_quiz_results:
            ...
            limit_minutes = quiz.get('time_limit', 10) 
           ...
           # --- 2. QUIZ FORM ---
            with st.form("quiz_form"):
                # Use shuffled indices for questions
                for idx in st.session_state.shuffled_indices:
                    ...
                    # Retrieve the fixed shuffled order for this session
                    current_opts = st.session_state[opt_key]
                ...
                if cancel:
                    # Clear shuffled options even on cancel
                    for key in list(st.session_state.keys()):
                        if key.startswith("shuffled_opts_"):
                            del st.session_state[key]

                    # Standard cleanup
                    if "quiz_start_time" in st.session_state: del st.session_state.quiz_start_time
                    if "shuffled_indices" in st.session_state: del st.session_state.shuffled_indices
                    del st.session_state.active_quiz
                    del st.session_state.quiz_answers
                    st.rerun()
                ...
                if submitted:
                    ...
                    # 5. Prepare the results view data
                    ...
                    # 6. Clean up main quiz state and refresh
                    if "quiz_start_time" in st.session_state: del st.session_state.quiz_start_time
                    if "shuffled_indices" in st.session_state: del st.session_state.shuffled_indices
                    st.rerun()
        else:
            ...
            if not quizzes:
                ...
                for quiz in quizzes:
                    ...
                    with col1:
                        ...
                        st.write(f"*Difficulty:* {quiz.get('difficulty', 'Medium')} | *Time:* {quiz.get('time_limit', 10)}m")
    ...
    with tab2:
        ...
        if not results:
            ...
            # 1. Display individual results in expanders
            ...
            # 2. Add the Export button at the bottom of the list
            st.markdown("---")
            st.markdown("### 📥 Export Your Progress")
            df_results = pd.DataFrame(results)
            
            # Clean up the dataframe for better readability in Sheets
            if not df_results.empty:
                # Convert the '_id' column to string to avoid JSON errors during CSV export
                df_results['_id'] = df_results['_id'].astype(str)
                
                csv = df_results.to_csv(index=False).encode('utf-8')
                st.download_button(
                    label="Download All Results as CSV (Google Sheets)",
                    data=csv,
                    file_name=f"{st.session_state.username}_legal_quiz_results.csv",
                    mime="text/csv",
    ...
    with tab3:
        ...
        quizzes = get_all_quizzes()
        selected_quiz = st.selectbox("Select Quiz to view Rankings", quizzes, format_func=lambda x: x['title'])
        
        if selected_quiz:
            lb_data = get_leaderboard(selected_quiz['_id'])
            if lb_data:
                # 1. Trigger the 5-second animation only once after submission
                if st.session_state.get("celebrate_podium", False):
                    st.balloons() # Native animation
                    st.toast(f"🎉 Congrats {st.session_state.username}! You're on the podium!", icon="🥇")
                    # Remove flag immediately so it doesn't repeat every second
                    del st.session_state.celebrate_podium 

                # 2. Render the styled table
                df_lb = pd.DataFrame(lb_data)
                
                # Safety check for old data
                if 'time_taken' not in df_lb.columns:
                    df_lb['time_taken'] = "N/A"
                
                # Format for display
                df_lb = df_lb[["username", "score", "time_taken"]].rename(columns={
                    "username": "User", 
                ...
                # The CSS in apply_global_styles() handles Gold/Silver/Bronze colors
                st.table(df_lb)
            else:
                st.info("No participants yet. Be the first to top the chart!")
